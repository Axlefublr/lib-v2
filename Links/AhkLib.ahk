Links_AhkLib := Map(

   "ahk v1 docs",
   "https://www.autohotkey.com/docs/Tutorial.htm",

   "ahk wiki",
   "https://autohotkey.wiki/",

   "v2 download",
   "https://www.autohotkey.com/download/2.0/",

   ;; aliztori

   "aliztori mouseinzone",
   "https://github.com/aliztori/AlizMod/blob/main/MouseInZone.ahk",

   ;; thqby

   "thqby libs",
   "https://github.com/thqby/ahk2_lib",

   "thqby sort",
   "https://github.com/thqby/ahk2_lib/blob/master/sort.ahk",

   "thqby print",
   "https://github.com/thqby/ahk2_lib/blob/master/print.ahk",

   "thqby json",
   "https://github.com/thqby/ahk2_lib/blob/master/JSON.ahk",

   "thqby chrome.ahk",
   "https://github.com/thqby/ahk2_lib/blob/master/Chrome.ahk",

   "thqby ahk extension",
   "https://github.com/thqby/vscode-autohotkey2-lsp",

   ;; Descolada

   "descolada libs",
   "https://github.com/Descolada/AHK-v2-libraries",

   "descolada string",
   "https://github.com/Descolada/AHK-v2-libraries/blob/main/Lib/String.ahk",

   "descolada array",
   "https://github.com/Descolada/AHK-v2-libraries/blob/main/Lib/Array.ahk",

   ;; reiwa

   "reiwa clock",
   "https://github.com/rbstrachan/ahk-clock",

   ;; TigerLily

   "screen dimmer",
   "https://github.com/tigerlily-dev/tigerlilys-Screen-Dimmer/",

)